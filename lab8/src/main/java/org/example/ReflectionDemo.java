...
public class ReflectionDemo
{
    ...
    public static int HowManyPeople(List<Object> objectList)
    {
        ...
        {
            if (object instanceof Human) //бинарный оператор получает на вход результат вычислений выражения (объект) и
            {                            //выдает true, если этот объект явл объектом класса <класс> или
                res++;                   //объектом производного от <класс> класса, false иначе
            }
        }
        return res;
    }
    ...
    {
        Class<?> clz = o.getClass();         //служебный класс, объекты которого хранят метаданные класса T(?)
        Method[] methods = clz.getMethods(); //загруженные в память Java-машины во время работы программы
    ...
    public static int ImplementsAnExecutable(List<Object> list)
    {
        ...
        for (Object o : list)
        {
            if (o instanceof Executable b) //является ли переменная экземпляром объекта указанного типа или экземпляром дочернего типа к указанному.
            {
                b.execute();
                res++;
            }
