...
public class CollectionsDemo
{
    ...
    public static ArrayList<Human> ListWithoutOne(ArrayList<Human> listHuman, Human one)
    {
        ...
        res.remove(one); //удаление объекта по ссылке
        return res;
    }
    /**
     * 4
    ...
    public static List<Set<Integer>> NotIntersect(List<Set<Integer>> listSet, Set<Integer> set) {
        ...
        for (Set<Integer> setInt : listSet) {
            Set<Integer> intersection = new HashSet<>(setInt); //HashSet (эл-ты мн-ва не упорядочны) не позволяет хранение одинаковвые объеты\эл-ты
            intersection.removeAll(set); //HashSet инкапсулирует в себе HashMaP (т.е. использует для храннеия хэш-таблицу)
     ...
     */
    public static List<Integer> ListOfAdults(Map<Intege, Human> map) {
